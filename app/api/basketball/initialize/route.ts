...
export async function POST(request: NextRequest) {
  try {
    ...
    await prisma.basketballMatch.deleteMany({
      where: {
        gender: gender
      }
    })
    ...
    await prisma.basketballRanking.deleteMany({
      where: {
        gender: gender
      }
    })
    ...
    return NextResponse.json({
      message: `${gender === 'men' ? '男子' : '女子'}バスケットボールトーナメントを初期化しました`,
      matchesCreated: createdMatches.count
    })

  } catch (error) {
    console.error('Error initializing basketball tournament:', error)
    return NextResponse.json(
