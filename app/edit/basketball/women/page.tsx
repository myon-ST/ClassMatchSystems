...
export default function WomenBasketballEdit() {
  ...
  const fetchMatches = async () => {
    try {
      setIsLoading(true)
      const response = await fetch('/api/basketball/matches?gender=women')
      if (response.ok) {
  ...
  const advanceWinner = async (match: BasketballMatch, winner: string) => {
    // 正しいトーナメント進出ロジックを実装（女子用）
    const advancementMap: { [key: string]: { nextMatch: string, position: 'team1' | 'team2' } } = {
      'BWA1': { nextMatch: 'BWA5', position: 'team2' },
      'BWB1': { nextMatch: 'BWB5', position: 'team1' },
      'BWB2': { nextMatch: 'BWB6', position: 'team1' },
      'BWA2': { nextMatch: 'BWA6', position: 'team2' },
      'BWA3': { nextMatch: 'BWA6', position: 'team1' },
      'BWA4': { nextMatch: 'BWA7', position: 'team2' },
      'BWA5': { nextMatch: 'BWA7', position: 'team1' },
      'BWB3': { nextMatch: 'BWB7', position: 'team1' },
      'BWB4': { nextMatch: 'BWB8', position: 'team1' },
      'BWB5': { nextMatch: 'BWB7', position: 'team2' },
      'BWB6': { nextMatch: 'BWB8', position: 'team2' },
      'BWA6': { nextMatch: 'BWB9', position: 'team2' },
      'BWA7': { nextMatch: 'BWB9', position: 'team1' },
      'BWB7': { nextMatch: 'BWB10', position: 'team1' },
      'BWB8': { nextMatch: 'BWB10', position: 'team2' },
      'BWB9': { nextMatch: 'BWA8', position: 'team1' },
      'BWB10': { nextMatch: 'BWA8', position: 'team2' }
    }
    ...
    if (match.matchCode === 'BWB9' || match.matchCode === 'BWB10') {
    ...
    // 準決勝の場合は3位決定戦の進出者も更新
    if (match.matchCode === 'BWB9' || match.matchCode === 'BWB10') {
      const newLoser = match.team1 === winner ? match.team2 : match.team1
  ...
  const updateRankingsAutomatically = async (finishedMatch: BasketballMatch, winner: string) => {
    try {
      ...
      // 準決勝の場合は敗者を3位決定戦に進出させる
      if (finishedMatch.matchCode === 'BWB9' || finishedMatch.matchCode === 'BWB10') {
        const loser = finishedMatch.team1 === winner ? finishedMatch.team2 : finishedMatch.team1
        await advanceLoserToThirdPlace(finishedMatch.matchCode, loser)
      }
    } catch (error) {
  ...
  const advanceLoserToThirdPlace = async (matchCode: string, loser: string) => {
    const thirdPlaceMatch = tournament.matches.find(m => m.matchCode === 'BWB11')
    if (thirdPlaceMatch) {
      const position = matchCode === 'BWB9' ? 'team1' : 'team2'
      const updateData: any = {}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">🏀 女子バスケットボール</h1>
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-8">
                          ...
                          <div className="flex gap-1 sm:gap-2 w-full sm:w-auto">
                            <div className="flex-1 sm:flex-none">
                              ...
                              <input
                                ...
                                className="w-full sm:w-12 lg:w-16 h-8 text-center border rounded focus:ring-2 focus:ring-pink-500 text-xs sm:text-sm"
                            ...
                            <div className="flex-1 sm:flex-none">
                              ...
                              <div className="w-full sm:w-12 lg:w-16 h-8 flex items-center justify-center font-bold text-pink-600 bg-pink-50 rounded border text-xs sm:text-sm">
                          ...
                          <div className="flex gap-1 sm:gap-2 w-full sm:w-auto">
                            <div className="flex-1 sm:flex-none">
                              <input
                                ...
                                className="w-full sm:w-12 lg:w-16 h-8 text-center border rounded focus:ring-2 focus:ring-purple-500 text-xs sm:text-sm"
                                disabled={!match.team1 || !match.team2 || (match.status !== 'in_progress' && !match.isEditing)}
                              />
                            </div>
                            <div className="flex-1 sm:flex-none">
                            ...
                            <div className="flex-1 sm:flex-none">
                              <div className="w-full sm:w-12 lg:w-16 h-8 flex items-center justify-center font-bold text-purple-600 bg-purple-50 rounded border text-xs sm:text-sm">
        ...
        {/* 順位表 */}
        <BasketballRankings gender="women" />
      </div>
    </div>
  )
} 